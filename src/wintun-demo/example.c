...
{
    HMODULE Wintun =
            LoadLibraryExW(L"D:\\Tools\\msys64\\home\\SuHaitao\\lab\\tun_lab\\lib\\wintun.dll", NULL, LOAD_LIBRARY_SEARCH_APPLICATION_DIR | LOAD_LIBRARY_SEARCH_SYSTEM32);
    if (!Wintun)
        return NULL;
...
MakeICMP(_Out_writes_bytes_all_(28) BYTE Packet[28])
{
    ...
    *(USHORT *)&Packet[2] = htons(28);
    Packet[8] = 255;
    Packet[9] = 1;
    *(ULONG *)&Packet[12] = htonl((10 << 24) | (0 << 16) | (0 << 8) | (2 << 0)); /* 10.6.7.8 */
    *(ULONG *)&Packet[16] = htonl((10 << 24) | (0 << 16) | (0 << 8) | (1 << 0)); /* 10.6.7.7 */
    *(USHORT *)&Packet[10] = IPChecksum(Packet, 20);
    Packet[20] = 8;
    *(USHORT *)&Packet[22] = IPChecksum(&Packet[20], 8);
    Log(WINTUN_LOG_INFO, L"Sending IPv4 ICMP echo request from 10.6.7.8 to 10.6.7.7");
}
...
int __cdecl main(void)
{
    ...
    AddressRow.Address.Ipv4.sin_addr.S_un.S_addr = htonl((10 << 24) | (0 << 16) | (0 << 8) | (1 << 0)); /* 10.6.7.7 */
